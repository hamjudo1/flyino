...
const int leftReflectionDPin = 2;
const int leftEnableDPin = 3;
const int rightReflectionDPin = 12;
const int rightEnableDPin = 11;
const int centerReflectionDPin = 6;
const int centerEnableDPin = 5;
const int upperMotor = 9; // digital D9
const int lowerMotor = 10; // digital D10
// LED is on D13
...
const int OBSTACLE_STRENGTH_ADDR = 76;
...
cmdStruct cStructs[] =
{
	...
	{OBSTACLE_STRENGTH_ADDR, "U", 0,0, "obstacle strength factor (if 0.0 ignore obstacle sensors)", eepromWriteUserFloat},
...
{
	//trimGyro();
	pinMode(leftReflectionDPin, INPUT_PULLUP);
	pinMode(leftEnableDPin, OUTPUT);
	pinMode(rightReflectionDPin, INPUT_PULLUP);
	pinMode(rightEnableDPin, OUTPUT);
	pinMode(centerReflectionDPin, INPUT_PULLUP);
	pinMode(centerEnableDPin, OUTPUT);
	digitalWrite(centerEnableDPin, 1);
	digitalWrite(leftEnableDPin, 1);
	digitalWrite(rightEnableDPin, 1);
...
void allSensorTest()
{
	...
	Serial.print(! digitalRead(leftReflectionDPin));
	Serial.print(! digitalRead(centerReflectionDPin));
	Serial.print(! digitalRead(rightReflectionDPin));
	Serial.println();
}
...
void hoverGyroTest()
{
	...
	float obstacleStrength = eepromReadFloat(OBSTACLE_STRENGTH_ADDR);
	//TODO needs work
	...
	while (now < endTime)
	{
		if ( now > nextIter )
		{
			...
			boolean centerObstruction = ! digitalRead(centerReflectionDPin);
			boolean rightObstruction = ! digitalRead(rightReflectionDPin);
			boolean leftObstruction = ! digitalRead(leftReflectionDPin);
			float targetGyro = startGyro;
			if ( USE_OBSTRUCTION )
			{
				if ( leftObstruction )
				{
					leftRange = obstacleStrength;
				} else {
					leftRange = 150.0;
				}
				if ( rightObstruction ) 
				{
					rightRange = obstacleStrength;
				} else {
					rightRange = 150.0;
				}
			}
...
{
	setMotorSpeeds(100.0, 100.0);
	if ( secondsOfFlight < 1 || secondsOfFlight > 240)
	{
		secondsOfFlight = 1;
	}
	delay((long)1000 * (long)secondsOfFlight);
	setMotorSpeeds(0.0, 0.0);
	logVoltageAndGyroMinMax();
}
...
{
	Serial.print(F("Reflectance test |"));
	while( 0 == Serial.available())
	{
		for(int left = 0; left < 2; left ++)
		{
			...
			{
				digitalWrite(leftEnableDPin, left);
				digitalWrite(rightEnableDPin, right);
				delay(200);
				Serial.print(left);
				Serial.print(right);
				readDisplayReflectance(leftReflectionDPin);
				readDisplayReflectance(rightReflectionDPin);
				Serial.print(F(" | "));
			}
		}
		Serial.println();
